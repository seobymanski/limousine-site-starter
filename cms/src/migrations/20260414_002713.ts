...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`blog_posts_related_posts\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`blog_posts_related_posts_order_idx\` ON \`blog_posts_related_posts\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`blog_posts_related_posts_parent_id_idx\` ON \`blog_posts_related_posts\` (\`_parent_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`blog_posts_related_posts\`;`)
