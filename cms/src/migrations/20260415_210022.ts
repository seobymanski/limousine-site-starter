...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`blog_posts_gallery_images\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`blog_posts_gallery_images_order_idx\` ON \`blog_posts_gallery_images\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`blog_posts_gallery_images_parent_id_idx\` ON \`blog_posts_gallery_images\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`blog_posts_gallery_images_image_idx\` ON \`blog_posts_gallery_images\` (\`image_id\`);`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_blog_posts\`("id", "title", "slug", "excerpt", "body", "category", "author_name", "author_role", "author_image", "published_date", "status", "featured_image_id", "reading_time", "seo_meta_title", "seo_meta_description", "updated_at", "created_at") SELECT "id", "title", "slug", "excerpt", "body", "category", "author_name", "author_role", "author_image", "published_date", "status", "featured_image_id", "reading_time", "seo_meta_title", "seo_meta_description", "updated_at", "created_at" FROM \`blog_posts\`;`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`blog_posts_gallery_images\`;`)
  await db.run(sql`PRAGMA foreign_keys=OFF;`)
  await db.run(sql`CREATE TABLE \`__new_blog_posts\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
